...
class SchemaDiscovery
{
    ...
    private function fileToClassName(string $prefix, string $dir, string $file): ?string
    {
        ...
        $rel = substr($rel, 0, -4);
        $fqcn = $prefix . str_replace('/', '\\', $rel);

        // A PSR-4 prefix can map to a directory that also contains source for
        // MORE-SPECIFIC prefixes (e.g. the application package root holds
        // DbSchema/src + SearchCriteria/src under their own Flyokai\DbSchema\* /
        // Flyokai\SearchCriteria\* prefixes). Naive path->class then yields a bogus
        // FQCN (Flyokai\Application\DbSchema\src\...) that the catch-all autoloader
        // resolves back to the real file, re-including it under a different name ->
        // "Cannot redeclare". Only accept the FQCN when the file actually declares
        // the matching namespace.
        $expectedNs = ($pos = strrpos($fqcn, '\\')) !== false ? substr($fqcn, 0, $pos) : '';
        $declaredNs = $this->declaredNamespace($real);
