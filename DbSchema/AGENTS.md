...
- **Identity metadata duplication**: each DTO trait still declares `static $idKey` / `$altIdKeys` / `$type` for `AbstractRepository`. Phase 2 will unify these with the schema attributes via a `SOLID_CLASS` constant on the shared trait.
